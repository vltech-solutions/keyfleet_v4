@props([
    'value' => 0,      // Fuel percentage 0-100
    'blocks' => 10     // Number of bars
])

<div
    x-data="{
        state: Number(@json($value)),
        totalBlocks: {{ $blocks }},
        ...
        getBlockColor(blockIndex) {
            ...
            if (this.state < threshold) return 'bg-gray-200 dark:bg-gray-800';
...
>
    ...
    <div class="mb-2 text-xs font-semibold text-gray-600 flex justify-between">
        ...
        <span :class="state <= 20 ? 'text-red-500' : 'text-emerald-500'" x-text="displayValue"></span>
    </div>

    <!-- Horizontal Bars -->
    <div class="flex gap-1">
        <template x-for="i in totalBlocks" :key="i">
            <div class="flex-1 h-3 rounded-sm transition-all" :class="getBlockColor(i)"></div>
        </template>
    </div>
